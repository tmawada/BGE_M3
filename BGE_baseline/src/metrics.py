...
Implements Recall@K, MRR, and nDCG@K from scratch.
"""
...
def recall_at_k(
    ...
    Returns:
        Mean Recall@K across all queries with relevance judgments.
    """
    ...
    for qid, relevant_docs in qrels_dict.items():
        ...
        retrieved_at_k: List[str] = results[qid][:k]
...
def mrr(
    ...
    MRR = average of 1/rank of the first relevant document across all queries.
...
def ndcg_at_k(
    ...
    Returns:
        Mean nDCG@K across all queries with relevance judgments.
    """
    qrels_dict = _build_qrels_dict(qrels)
