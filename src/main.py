"""
Main pipeline orchestrator for BGE-M3 dense retrieval baseline.

Supports running the full pipeline or individual steps via CLI arguments.
...
def run_full_pipeline(config: Dict[str, Any]) -> None:
    ...
    print("\n" + "#" * 60)
    print("  BGE-M3 Dense Retrieval Baseline Pipeline")
    print("#" * 60)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="BGE-M3 Dense Retrieval Baseline Pipeline"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    elif args.step == "encode_corpus":
        step_encode_corpus(config)
    elif args.step == "encode_query":
        step_encode_query(config)
    elif args.step == "build_index":
        step_build_index(config)
    elif args.step == "retrieve":
        step_retrieve(config)
