...
def load_mlm_model(config: Dict[str, Any]) -> tuple:
    """Load BGE-M3 as a masked language model for the DAPT stage.
    ...
    are fully reused). The tokenizer is reused as-is.
...
def load_model(config: Dict[str, Any]) -> Encoder:
    ...
    If `use_adapter` is True in the config, loads the DAPT checkpoint (or
    `model_name` fallback) as the frozen base with PEFT adapter weights from
    `adapter_path`. Otherwise, loads the vanilla BGE-M3 model via
    FlagEmbedding using `base_model_name`.

    Args:
        config: Configuration dictionary. Keys: 'base_model_name',
            'dapt_model_dir', 'model_name', 'device', 'use_fp16',
            'use_adapter', 'adapter_path'.
    ...
    if use_adapter:
        base_model: str = (
            config.get("dapt_model_dir")
            or config.get("model_name", "BAAI/bge-m3")
        )
        adapter_path: str = config.get("adapter_path", "adapters/bge-m3-dapt-lora")
